...
exports.protect = catchAsync(async (req, res, next) => {
  // 1) Getting token and check exist
  let token;
  if (
...
exports.forgotPassword = catchAsync(async (req, res, next) => {
  ...
  try {
    ...
    return next(
      new AppError('There was an error sendEmail. Try again later', 500)
    );
  }
});
...
exports.updatePassword = catchAsync(async (req, res, next) => {
  ...
  const currentPass = req.body.passwordCurrent;
  const correct = await user.correctPassword(currentPass, user.password);
